...
import { MockConfigService, MockRepository } from '@rspd/shared/backend/test-util';
...
describe('UserMailService', () => {
	...
	beforeEach(async () => {
		...
		emailRepository = module.get(getRepositoryToken(Mail));
	...
	describe('create', () => {
		...
		it('should throw a `DuplicateSourceException` in case an email is already registered', async () => {
			...
			await expect(service.create(email)).rejects.toThrowError(DuplicateSourceException);
		});
	});
	...
	describe('sendConfirmationMail', () => {
		it('should send an email with confirmation link', async () => {
			...
			const confirmationLink = `${configService.get('url')}:${configService.get(
				'port',
		...
		it('should throw BadRequestException if email is already confirmed', async () => {
			...
			await expect(service.sendConfirmationMail(testString, 'wrong')).rejects.toThrow(
				BadRequestException,
			);
		});
	});
	...
	describe('setMailToConfirmed', () => {
		it('should set isEmailValidated field to true for an existing email', async () => {
			...
			const result = await service.setMailToConfirmed(testEmailString);

			expect(findSpy).toHaveBeenCalledWith(testEmailString);
		...
		it('should throw an ActionNotPerformedException for a non-existing email', async () => {
			await expect(service.setMailToConfirmed('not_found')).rejects.toThrow(
				NotFoundException,
			);
		});
	});
